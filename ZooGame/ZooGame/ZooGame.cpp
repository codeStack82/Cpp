...
void goToGame(int gameSelection, int argc, char** argv);
...
int guessAnimal(int argc, char **argv, int scores[], string players[]);
void howTo();
void maiinMenu();
void suubMenu();
int theBeginning(int argc, char** argv);

//Sean
void addition(string players[]);
void subtraction(string players[]);
void multi(string players[]);
void checkScore(string players[]);
void choice();
int score;
int chances = 5;
int bank[2] = { 0,0 };
int playerOnechance = 5;
int playerTwochance = 5;
int turn = 0;
...
int main(int argc, char** argv)
{
	...
	goToGame(gameselection, argc,argv);
	return 0;
}
void greetingMsg()
{   //Contract:
...
{   //Contract 
	...
	cout << "\t (3) to Play the Math Game!" << endl;
...
void goToGame(int gameNum, int argc, char** argv)
{  //Contract 
   //  @notes:     Display options, prompt user to make selection 
   //  @params     int, result
   //  @return     int, user selection
...
void DiiceGame(int * scores, string * players)
{
	...
	while (1) {
		...
		if (scores[0] >= 10000) {
			cout << " Congrats Player " << players[0] << " You Win!!!" << endl;
			break;
		}
		else if (scores[1] >= 10000) {
			cout << " Congrats Player " << players[1] << " You Win!!!" << endl;
			break;
		}
...
void useYourBrain() {
	...
	while (chances > 0) {
		...
		while (!(cin >> option)) {
			cout << "Bad input!\n";
			cout << "Please select the choice again " << endl;
			cin.clear();
			cin.ignore(numeric_limits<streamsize>::max(), '\n');
		}
		switch (option) {
...
void choice() {
	...
	cout << "4 - Checking Score" << endl;
	cout << "5 - Exit" << endl;
	cout << endl << "Choice: ";

}
void checkScore(string * players) {
	cout << "The score for Player " << players[0] << ": " << bank[0] << endl;
	cout << "The score for Player " << players[1] << ": " << bank[1] << endl;
}
void addition(string * players)
{
	...
	bool flag = 1;
	do {
		if (turn % 2 == 0 && playerOnechance > 0) {
			cout << " It is turn for player: " << players[0] << endl;
		}
		else if (playerTwochance > 0) {
			cout << " It is turn for player: " << players[1] << endl;
		}
		srand((unsigned)time(0));
		int n1 = rand() % 25 + 1;
		int n2 = rand() % 25 + 1;
		...
		if (answer == n1 + n2)
		{
			...
			if (turn % 2 == 0 && playerOnechance > 0) {
				cout << " Player: " << players[0] << " chances is " << playerOnechance << endl;
				bank[0] += 10;
				cout << " Score of player: " << bank[0] << endl;
			}
			else if (playerTwochance > 0) {
				cout << " Player: " << players[1] << " chances is " << playerTwochance << endl;
				bank[1] += 10;
				cout << " Score of player: " << bank[1] << endl;
			}
			count++;
			turn++;
		}
	...
	} while (count != 5 && playerOnechance > 0 || playerTwochance > 0);
	...
	if (count >= 5) {
		...
		do {
			...
			else
			{
				cout << endl << "Wrong! The right answer was " << n1 + n2 << endl << endl;
				//chances--;
				//cout << " Your chances: " << chances << endl;
				if (turn % 2 == 0 && playerOnechance > 0) {
					playerOnechance--;
					cout << " Player: " << players[0] << " chances is " << playerOnechance << endl;
					bank[0] -= 5;
	...
	if (count >= 5) {
		...
		do {
			...
			else
			{
				...
				if (turn % 2 == 0 && playerOnechance > 0) {
					playerOnechance--;
					cout << " Player: " << players[0] << " chances is " << players[0] << endl;
					bank[0] -= 5;
					cout << " Score of player: " << bank[0] << endl;
				}
				else if (playerTwochance > 0) {
		...
		} while (count != 10 && playerOnechance > 0 || playerTwochance > 0);
	}
	...
	if (count >= 10) {
		...
		do {
			if (turn % 2 == 0 && playerOnechance > 0) {
				...
				cout << " It is turn for player: " << players[2] << endl;
			}
			srand((unsigned)time(0));
			int n1 = rand() % 100 + 1;
			int n2 = rand() % 100 + 1;
		...
		} while (count != 15 && playerOnechance > 0 || playerTwochance > 0);
	}
}
void multi(string * players)
{
	...
	if (count >= 10) {
		...
		} while (playerOnechance > 0 || playerTwochance > 0);
	}
}
...
int theBeginning(int argc, char **argv) {
	...
	while (1) {
		...
		case 1:
			...
			break;
		case 2:
			guessAnimal(argc, argv, bank, players);
			break;
		case 3:
			cout << players[0] << " : " << bank[0] << '\n';
			cout << players[1] << " : " << bank[1] << '\n';
...
int guessAnimal(int argc, char **argv, int * scores, string * players) {
	...
	cout << players[0] << " and " << players[1] << ", you have 10 chances a piece to guess the animal!" << endl << endl;
				...
				for (auto it = letterPositions.begin();
					...
					if (currentGuess == it->first) {
						for (int position : it->second) {
							...
							scores[0] += 5000;
							cout << " player " << players[0] << " scored: " << scores[0] << endl;
							if (wordVector == guessStatus) {
								cout << "Congratulations! You guessed the animal!!!" << endl;
								cout << "Final Scores: " << endl;
								cout << "Player One: " << scores[0] << endl;
								cout << "Player Two: " << scores[1] << endl;
								cout << " Congrats You win " << players[0] << endl;
				...
				for (auto it = letterPositions.begin();
					...
					if (currentGuess == it->first) {
						for (int position : it->second) {
							guessStatus.at(position) = currentGuess;
							scores[1] += 5000;
							cout << " player " << players[1] << " scored: " << scores[1] << endl;
							if (wordVector == guessStatus) {
								...
								if (scores[0] > scores[1]) {
									...
									cout << " Congrats " << players[0] << " and " << players[1] << " you both are winners " << endl;
								}
								else {
									cout << " Congrats You win " << players[1] << endl;
								}
...
void howTo() {
	string howToPlay = "\"HOW TO PLAY GUIDE\":\n"
		"\tGuessAnimal is played by two players, with each player in succession having a turn at \n"
		"\tYou have to guess what animal it could be. Each players will have 20 chances to guess\n"
		"\tplayer to guess the animal first wins.\n"
		"\n\n";
	cout << howToPlay << endl;
...
void suubMenu() {
	cout << "\n\n";
	cout << "Would you like to continue to play again (y/n)?: ";
}
